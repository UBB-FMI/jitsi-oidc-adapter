...
export class MeetingStore {
  ...
  async init(): Promise<void> {
    let migrated = false;
    try {
      const parsed = JSON.parse(await Deno.readTextFile(SCHEDULER_DATA_FILE)) as Meeting[];
  ...
  async sweep(): Promise<void> {
    ...
    for (const meeting of this.#meetings.values()) {
      if (meeting.finishedAt || !meeting.hasEverJoined) continue;
      if (this.#activeFor(meeting.id)) {
  ...
  #persist(): Promise<void> {
    this.#writeQueue = this.#writeQueue.then(async () => {
      const temporary = `${SCHEDULER_DATA_FILE}.tmp`;
      await Deno.writeTextFile(temporary, JSON.stringify([...this.#meetings.values()], null, 2));
      await Deno.rename(temporary, SCHEDULER_DATA_FILE);
    });
...
export function landingPage(): string {
  return page("Welcome", `<section class="card"><h1>Meetings hosted by our faculty.</h1><div class="grid"><article class="choice"><h2>Join a meeting</h2><p>Paste a meeting link or enter its 12-character code.</p><form id="join" class="stack"><div class="field"><label for="meeting">Meeting link or code</label><input id="meeting" autocomplete="off" required placeholder="https://meet.cs.ubbcluj.ro/m/…"></div><div id="join-error" class="hint" role="alert"></div><button type="submit">Join meeting</button></form></article><article class="choice"><h2>Create a meeting</h2><p>Start now or schedule a link to share. Microsoft login is required to host.</p><a class="btn" href="/fmi/new">Create a meeting</a></article></div></section>`, `<script>document.getElementById('join').addEventListener('submit',e=>{e.preventDefault();const v=document.getElementById('meeting').value.trim(),valid=new RegExp('^[a-f0-9]{12}$','i');let code=valid.test(v)?v:'';if(!code){try{const u=new URL(v,location.origin),parts=u.pathname.split('/').filter(Boolean);if(parts.length===2&&parts[0]==='m'&&valid.test(parts[1]))code=parts[1]}catch{}}if(code)location.href='/m/'+code.toLowerCase();else document.getElementById('join-error').textContent='Please enter a valid FMI Meets link or code.'})</script>`);
}

export function newMeetingPage(): string {
  return page("Create a meeting", `<section class="card"><h1>Create a meeting</h1><p class="lead">You will sign in with your UBB Microsoft account before the meeting is created.</p><form class="stack" method="post" action="/fmi/create" id="create"><div class="field"><label for="title">Meeting title</label><input id="title" name="title" maxlength="120" required placeholder="Department meeting"></div><div class="field"><label for="mode">When</label><select id="mode" name="mode"><option value="now">Start now</option><option value="schedule">Schedule for later</option></select></div><div class="field" id="scheduled" hidden><label for="localStart">Start date and time</label><input id="localStart" type="datetime-local"><span class="hint" id="zone"></span></div><input type="hidden" name="startIso" id="startIso"><input type="hidden" name="timeZone" id="timeZone"><button type="submit">Continue with Microsoft</button></form></section>`, `<script>const mode=document.getElementById('mode'),box=document.getElementById('scheduled'),local=document.getElementById('localStart'),zone=Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';document.getElementById('timeZone').value=zone;document.getElementById('zone').textContent='Your timezone: '+zone;const min=new Date(Date.now()+5*60000);min.setMinutes(min.getMinutes()-min.getTimezoneOffset());local.min=min.toISOString().slice(0,16);mode.onchange=()=>{box.hidden=mode.value!=='schedule';local.required=mode.value==='schedule'};document.getElementById('create').onsubmit=e=>{if(mode.value==='schedule'){const d=new Date(local.value);if(!Number.isFinite(d.getTime())||d.getTime()<Date.now()){e.preventDefault();alert('Choose a future start time.');return}document.getElementById('startIso').value=d.toISOString()}}</script>`);
}

export function scheduledCreatedPage(meeting: Meeting, publicUrl: string): string {
  const link = `${publicUrl}/m/${meeting.id}`;
  return page("Meeting scheduled", `<section class="card"><h1>Meeting scheduled</h1><p class="lead">${escapeHtml(meeting.title)}</p><div class="notice"><div>Starts</div><div class="time local-time" data-time="${meeting.startAt}"></div><div class="meta">Scheduled in ${escapeHtml(meeting.creatorTimeZone)}</div></div><p>Share this link with attendees:</p><div class="meeting-link"><input id="link" readonly value="${escapeHtml(link)}"><button id="copy" type="button">Copy</button></div><p><a class="btn" href="/fmi/host/${meeting.id}">Enter as moderator</a> <a class="btn secondary" href="/">Done</a></p></section>`, timeScript(`document.getElementById('copy').onclick=async()=>{await navigator.clipboard.writeText(document.getElementById('link').value);document.getElementById('copy').textContent='Copied'}`));
}
...
export function finishedPage(meeting: Meeting): string {
  return page("Meeting finished", `<section class="card"><h1>Meeting finished</h1><p class="lead">${escapeHtml(meeting.title)}</p><p>This meeting has been empty for more than ${MEETING_INACTIVE_HOURS} hours and its link is now closed.</p><a class="btn" href="/">Return to FMI Meets</a></section>`);
}

export function meetingPage(meeting: Meeting, hostJwt = ""): string {
  const room = `_fmi_${meeting.room}`;
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(meeting.title)} · FMI Meets</title><style>html,body,#meet{height:100%;width:100%;margin:0;background:#071d2a}iframe{display:block}</style></head><body><div id="meet"></div><script src="/external_api.js"></script><script>
